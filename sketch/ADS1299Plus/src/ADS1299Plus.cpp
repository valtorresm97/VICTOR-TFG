...
bool ADS1299Plus::begin()
{
  ...
  // 7) Verificar que el ADS detectado es la variante de 4 canales
  if ((id & ADS_ID_NU_CH_MASK) != 0b00) {
    return false;
  }
  
  num_channels_ = NUM_CHANNELS;  // coherencia interna: este driver está fijado a 4 canales
