...
La carpeta `docs/02_auditoria_codigo/auditoria_sistema/` no debe tratarse como duplicado simple de `docs/02_auditoria_codigo/funcion_por_funcion/`.

Aunque nacio como auditoria de `firmware-final-v1`, contiene descripciones narrativas y transversales de los sistemas que pueden ser muy utiles para redactar la memoria del TFG. La carpeta `02_auditoria_codigo/funcion_por_funcion/`, en cambio, es mas tabular, mas exhaustiva por funcion y mas orientada a no romper contratos durante refactor.

Decision final-v4:

```text
Conservar docs/02_auditoria_codigo/auditoria_sistema/
Reajustarla a firmware-final-v4
Usarla como capa narrativa/transversal para redaccion TFG
Usar docs/02_auditoria_codigo/funcion_por_funcion/ como capa tecnica funcion por funcion
```
...
| --- | --- | --- | --- |
| `docs/02_auditoria_codigo/funcion_por_funcion/` | Tablas, funcion por funcion, contratos, criticidad, pruebas | Seguridad tecnica para tocar codigo, contratos intocables, ruta esencial UML, candidatos a simplificacion | Refactor, UML tecnico, pruebas, evitar romper runtime |
| `docs/02_auditoria_codigo/auditoria_sistema/` | Texto mas global y narrativo por subsistema | Explicacion de arquitectura, papel de cada bloque, vision de sistema, contexto de decisiones | Redaccion TFG, explicacion de bloques, introduccion tecnica por modulos |
...
La carpeta `02_auditoria_codigo/auditoria_sistema/` debe actualizarse para no contradecir final-v4, pero no tiene que repetir tabla por tabla toda la auditoria detallada.
...
| Documento en `02_auditoria_codigo/auditoria_sistema/` | Relacion con `02_auditoria_codigo/funcion_por_funcion/` | Valor para TFG | Accion final-v4 |
| --- | --- | --- | --- |
...
6. Referenciar `02_auditoria_codigo/funcion_por_funcion/` para tablas exhaustivas.
...
Para decidir que entra y que no entra:
  docs/02_auditoria_codigo/funcion_por_funcion/09, 10, 11

Para escribir explicaciones del TFG:
  docs/02_auditoria_codigo/auditoria_sistema/01, 02, 03, 04, 05, 07, 08, 09
```
...
La carpeta `docs/02_auditoria_codigo/auditoria_sistema/` debe conservarse porque aporta texto explicativo util para la memoria. La carpeta `docs/02_auditoria_codigo/funcion_por_funcion/` ya cubre la precision tecnica funcion por funcion. La estrategia correcta es:
